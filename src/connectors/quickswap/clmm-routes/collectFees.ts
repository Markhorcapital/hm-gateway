  ...
  fastify.post<{
    ...
    async (request) => {
      try {
        ...
        const positionManagerContract = new Contract(positionManagerAddress, IAlgebraV3PositionManagerABI, wallet);
        ...
        // Get token objects from position
        ...
        if (!token0Obj || !token1Obj) {
          throw fastify.httpErrors.badRequest('Could not resolve tokens from position');
        }
        ...
        // Collect fees
        logger.info(`Collecting fees from position ${positionAddress}: ${tokensOwed0} token0 + ${tokensOwed1} token1`);
        ...
        logger.info(`Fees collected successfully. Transaction hash: ${receipt.transactionHash}`);
        ...
        return {
          ...
          data: {
            ...
            baseFeeAmountCollected: formatTokenAmount(tokensOwed0.toString(), token0Obj.decimals),
            quoteFeeAmountCollected: formatTokenAmount(tokensOwed1.toString(), token1Obj.decimals),
          },
        };
      } catch (error) {
        logger.error('Collect fees failed:', error);
        throw fastify.httpErrors.internalServerError(`Collect fees failed: ${error.message}`);
      }
    },
  );
};
