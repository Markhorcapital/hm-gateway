...
export const quotePositionRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    ...
    async (request) => {
      try {
        const { network, poolAddress, baseTokenAmount, quoteTokenAmount, lowerPrice, upperPrice } = request.body;

        const networkToUse = network || 'polygon';

        // Validate essential parameters
        if (!poolAddress || !lowerPrice || !upperPrice) {
          throw fastify.httpErrors.badRequest('Missing required parameters: poolAddress, lowerPrice, upperPrice');
        }
        ...
        const quickswapInstance = await QuickSwap.getInstance(networkToUse);
        const baseTokenObj = quickswapInstance.getTokenByAddress(poolInfo.baseTokenAddress);
        const quoteTokenObj = quickswapInstance.getTokenByAddress(poolInfo.quoteTokenAddress);
        ...
        // Validate essential parameters
        if (!baseTokenAmount || !quoteTokenAmount || !lowerPrice || !upperPrice) {
          throw fastify.httpErrors.badRequest(
            'Missing required parameters: baseTokenAmount, quoteTokenAmount, lowerPrice, upperPrice',
          );
        }
        ...
        logger.info(
          `Position quote: ${baseTokenAmount} ${baseTokenObj.symbol} + ${quoteTokenAmount} ${quoteTokenObj.symbol} at price range ${lowerPrice}-${upperPrice}`,
        );
        ...
        throw fastify.httpErrors.internalServerError(`Position quote failed: ${error.message}`);
      }
    },
  );
};
