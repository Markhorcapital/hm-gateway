    ...
    async (request) => {
      try {
        ...
        if (slippagePct) {
          ...
          mintParams.amount0Min = baseTokenAmountWei
            .mul(Math.floor(slippageMultiplier * 1000))
            .div(1000)
            .toNumber();
          mintParams.amount1Min = quoteTokenAmountWei
            .mul(Math.floor(slippageMultiplier * 1000))
            .div(1000)
            .toNumber();
        }
        ...
        if (error.statusCode) {
          ...
          throw fastify.httpErrors.internalServerError(`Open position failed: ${error.message}`);
        }
      }
    },
  );
