...
export async function getQuickSwapClmmQuote(
  ...
  const quote = await quoteClmmSwap(quickswap, baseTokenObj, quoteTokenObj, amount, side, poolAddress, slippagePct);
...
async function quoteClmmSwap(
  ...
  try {
    ...
    if (!actualPoolAddress) {
      actualPoolAddress = await quickswap.findDefaultPool(baseToken.symbol, quoteToken.symbol, 'clmm');
    ...
    const feeTier = 1000; // Default to 0.3%
