            ...
            try {
                ...
                if (!positionAddress) {
                    throw fastify.httpErrors.badRequest(
                        'positionAddress must be provided',
                    );
                }

                // Get V3 position data using Algebra V3 NFT interface
                const positionInfo = await getV3PositionInfo(
                    quickswap,
                    positionAddress,
                    walletAddress,
                );
                ...
                if (e.statusCode) {
                    ...
                    throw fastify.httpErrors.internalServerError(
                        `Position info request failed: ${e.message}`,
                    );
                }
            }
        },
 ...
 */
async function getV3PositionInfo(
    quickswap: any,
    ...
    try {
        ...
        const quoteTokenAmount = formatTokenAmount(
            liquidityBN.mul(BigNumber.from(Math.floor(currentPrice * 1e18).toString())).div(BigNumber.from(10).pow(18)).toString(),
            token1Obj.decimals
        );
