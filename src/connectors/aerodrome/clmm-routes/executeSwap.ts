...
import { getAerodromeV3SwapRouterAddress, IAerodromeSwapRouterABI } from '../aerodrome.contracts';
import { AerodromeExecuteSwapRequest } from '../schemas';
...
export async function executeClmmSwap(
  ...
  // Find pool address
  const poolAddress = await aerodrome.findDefaultPool(baseToken, quoteToken, 'clmm');
  if (!poolAddress) {
  ...
  const routerAddress = getAerodromeV3SwapRouterAddress(network);

  logger.info(`Executing swap using Aerodrome SwapRouter:`);
  ...
  logger.info(`Tick spacing: ${quote.tickSpacing}`);
  ...
  if (currentAllowance.lt(amountNeeded)) {
    ...
    throw fastify.httpErrors.badRequest(
      `Insufficient allowance for ${quote.inputToken.symbol}. Please approve at least ${formatTokenAmount(amountNeeded, quote.inputToken.decimals)} ${quote.inputToken.symbol} (${quote.inputToken.address}) for the Aerodrome SwapRouter (${routerAddress})`,
    );
  }
  ...
  // Build swap parameters for Aerodrome (uses tickSpacing instead of fee)
  const swapParams = {
  ...
  try {
    if (isHardwareWallet) {
      ...
      const iface = new utils.Interface(IAerodromeSwapRouterABI);
      let data;
      ...
      const routerContract = new Contract(routerAddress, IAerodromeSwapRouterABI, wallet);
...
export const executeSwapRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Execute a swap on Aerodrome V3 CLMM using SwapRouter',
    ...
    async (request) => {
      try {
        const { walletAddress, network, baseToken, quoteToken, amount, side, slippagePct } =
          request.body as typeof AerodromeExecuteSwapRequest._type;
